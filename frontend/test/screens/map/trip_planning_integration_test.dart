...
void main() {
  ...
  setUp(() {
    ...
    when(() => mapSettingsCubit.state).thenReturn(
      const MapSettingsState(showAirports: true, showStations: true),
    );
    when(
  ...
  testWidgets('Trip planning flow: Add airport to trip', (tester) async {
    ...
    final newState = TripState(
      stops: [
        TripStop(
          location: LatLng(airport.latitude, airport.longitude),
          sequenceOrder: 0,
          airport: airport,
        ),
      ],
    ...
    expect(
      ...
      findsOneWidget,
      reason: 'MapScreen should be present',
    );
